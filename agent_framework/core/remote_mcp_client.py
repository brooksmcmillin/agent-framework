...
class RemoteMCPClient:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                try:
                    ...
                    logger.debug(f"Caught BaseExceptionGroup during streamable setup: {len(eg.exceptions)} exceptions")
                    self._streamable_context = None
                ...
                if http_error:
                    ...
                    if http_error.response.status_code in (401, 403) and attempt < max_retries:
                        if self.enable_oauth:
                            ...
                            raise ValueError(
                                f"Authentication failed (HTTP {http_error.response.status_code})\n\n"
                ...
                if not self.enable_oauth and is_auth_error:
                    ...
                    raise ValueError(
                        ...
                        "To obtain a new token, run:\n"
                        "  uv run python scripts/mcp_auth.py"
                    ) from (last_error if last_error else e)
    ...
    async def disconnect(self) -> tuple[int | None, Exception | None]:
        ...
        if self._session:
            try:
                ...
                logger.debug(f"Error disconnecting from MCP server (BaseExceptionGroup): {len(eg.exceptions)} exceptions")
                for exc in eg.exceptions:
                    if isinstance(exc, httpx.HTTPStatusError):
        ...
        if self._streamable_context:
            try:
                ...
                # Extract HTTP error from exception group during cleanup
                logger.debug(f"Error closing streamable HTTP connection (BaseExceptionGroup): {len(eg.exceptions)} exceptions")
                for exc in eg.exceptions:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        _ = await self.disconnect()  # Ignore return value during normal exit
    ...
    async def _retry_with_reauth(self, operation_name: str, operation_func):
        ...
        try:
            ...
            if self._is_auth_error(e):
                logger.warning(
                    f"{operation_name} failed with auth error: {e}. "
